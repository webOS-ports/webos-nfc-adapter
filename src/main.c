...
static guint setup_signalfd(void)
{
	...
	source = g_io_add_watch(channel, G_IO_IN | G_IO_HUP | G_IO_ERR | G_IO_NVAL,
	                        signal_handler, NULL);
...
int main(int argc, char **argv)
{
	...
	g_message("webOS NFC adapter %s", VERSION);

	signal_source = setup_signalfd();

	event_loop = g_main_loop_new(NULL, FALSE);

	service = nfc_service_create();
